...
export class PostService {
  ...
  constructor(
    ...
    @InjectRepository(PostVote) private readonly postVoteRepository: Repository<PostVote>,
    ...
    @Inject(forwardRef(() => ThreadService)) private readonly threadsService: ThreadService,
  ...
  async createInitialPost(
      ...
      this.logger.log(`Creating initial post for thread ${threadId} by user ${author.username}`);
  ...
  async createReply(createPostDto: PostForCreateDto, threadId: string, author: User): Promise<Post> {
      this.logger.log(`User ${author.username} attempting to reply to thread ${threadId}`);
      ...
      if (parentPostId) {
          parentPost = await this.postRepository.findOne({ where: { id: parentPostId, threadId: threadId } }); // Aynı konuda olmalı
          if (!parentPost) {
              this.logger.warn(`Parent post ${parentPostId} not found in thread ${threadId}`);
              throw new NotFoundException(`Parent post with ID ${parentPostId} not found in this thread.`);
          }
      }
      ...
      try {
        ...
        this.logger.log(`Reply post created with ID: ${savedPost.id} in thread ${threadId}`);

        // 5. İstatistikleri güncelle (IDEALDE LISTENER/SUBSCRIBER İLE YAPILMALI)
        await this.threadsService.updateThreadStatsOnNewPost(threadId, savedPost.id, savedPost.createdAt, author.id);
  ...
  async findOne(id: string): Promise<Post> {
      ...
      const post = await this.postRepository.findOne({
          where: { id },
          relations: ['author', 'thread', 'thread.category', 'parentPost', 'parentPost.author'] // Gerekli ilişkiler
        });
  ...
  async update(id: string, updatePostDto: PostForUpdateDto, user: User): Promise<Post> {
      this.logger.log(`User ${user.username} attempting to update post ID: ${id}`);
      const post = await this.postRepository.findOne({ where: { id }, relations: ['author'] }); // Yazarı kontrol için al
      ...
      if (!isOwner && !isAdminOrMod) {
           this.logger.warn(`User ${user.username} forbidden to update post ID: ${id}`);
      ...
      try {
          ...
          this.logger.log(`Post ID: ${id} updated successfully by user ${user.username}`);
          return updatedPost;
      } catch (error) {
  ...
  async remove(id: string, user: User): Promise<void> {
      this.logger.log(`User ${user.username} attempting to remove post ID: ${id}`);
      const post = await this.postRepository.findOne({ where: { id }, relations: ['author', 'thread'] }); // Thread bilgisi lazım
      ...
      if (!isOwner && !isAdminOrMod) {
          this.logger.warn(`User ${user.username} forbidden to remove post ID: ${id}`);
          throw new ForbiddenException('You do not have permission to remove this post.');
      }

      // Başlangıç postu silinemez (konuyu silmek gerekir)
      if (post.isThreadStarter) {
          this.logger.warn(`Attempt to remove the starting post (ID: ${id}) of thread ${post.threadId}`);
          throw new BadRequestException('Cannot delete the starting post of a thread. Delete the thread instead.');
      }
      ...
      try {
          ...
          this.logger.log(`Post ID: ${id} removed successfully by user ${user.username}`);
  ...
  // --- Oylama İşlemleri ---
  async vote(postId: string, user: User, value: 1 | -1): Promise<{ currentScore: number, userVote: number | null }> {
    this.logger.log(`User ${user.username} voting on post ${postId} with value ${value}`);
    ...
    try {
      ...
      const existingVote = await voteRepo.findOne({ where: { postId, userId: user.id } });
      ...
      if (existingVote) {
        ...
        if (existingVote.value === value) {
          ...
          if (value === 1) upvoteChange = -1; else downvoteChange = -1;
          currentUserVote = null; // Oyu kaldırıldı
           this.logger.debug(`Vote removed for user ${user.id} on post ${postId}`);
        } else {
          ...
          if (oldValue === 1) upvoteChange = -1; else downvoteChange = -1;
          if (value === 1) upvoteChange += 1; else downvoteChange += 1;
          currentUserVote = value; // Yeni oy
           this.logger.debug(`Vote updated for user ${user.id} on post ${postId} to ${value}`);
        }
      } else {
        // İlk defa oy veriyor
        ...
        if (value === 1) upvoteChange = 1; else downvoteChange = 1;
        currentUserVote = value; // Yeni oy
         this.logger.debug(`New vote created for user ${user.id} on post ${postId} with value ${value}`);
      }
      ...
      if (scoreChange !== 0 || upvoteChange !== 0 || downvoteChange !== 0) {
        await postRepo.update(postId, {
         ...
         this.logger.debug(`Post ${postId} stats updated: scoreChange=${scoreChange}, upvoteChange=${upvoteChange}, downvoteChange=${downvoteChange}`);
      }
  ...
  // --- Yorum İşlemleri ---
  async addComment(postId: string, user: User, content: string): Promise<PostComment> {
  ...
  async findCommentsByPost(postId: string, queryDto: FindCommentsQueryDto): Promise<{ data: PostComment[], total: number, page: number, limit: number }> {
       this.logger.log(`Workspaceing comments for post: ${postId}, page: ${queryDto.page}, limit: ${queryDto.limit}`);
       // Post var mı kontrolü
       const postExists = await this.postRepository.existsBy({ id: postId });
       if (!postExists) {
            throw new NotFoundException(`Post with ID ${postId} not found.`);
  ...
  async findLastPostInThread(threadId: string, excludePostId?: string): Promise<Post | null> {
    const queryBuilder = this.postRepository.createQueryBuilder('post')
        .where('post.threadId = :threadId', { threadId });
